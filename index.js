...
function addGamesToPage(games) {
    ...
    for (let game of games) {
        ...
        newDiv.innerHTML = `<img class="game-img" src=${game['img']} />
                            <b><p>${game['name']}</b>: ${game['description']}</p>`
...
const displayStr = `A total of ${totalPledged.toLocaleString("en-US")} has been raised for ${totalGames} games. Currently, ${unfundedGames} ${unfundedGames > 1 ? "games remain unfunded" : "game remains unfunded"}. We need your help to fund these amazing games!`

// create a new DOM element containing the template string and append it to the description container
const newParagraph = document.createElement("p")
newParagraph.textContent = displayStr;
newParagraph.classList.add('centered');
...
firstName.textContent = `${firstGame.name}: $${firstGame.pledged.toLocaleString('en-US')} pledged`;
firstGameContainer.appendChild(firstName)
// do the same for the runner up item
const secondName = document.createElement("p");
secondName.textContent = `${secondGame.name}: $${secondGame.pledged.toLocaleString('en-US')} pledged`;
